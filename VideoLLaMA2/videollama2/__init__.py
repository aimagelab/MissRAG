...
def model_init(model_path=None, **kwargs):
    ...
    model_name = "VideoLLaMA2-7B" #get_model_name_from_path(model_path)
...
def mm_infer(image_or_video, instruct, model, tokenizer, modal='video', prot_prompt = "", **kwargs):
    ...
    # 1. vision preprocess into half tensors
    if modal == 'text':
        tensor = None
    else:
    ...
    split_point = "You are a helpful assistant."
    if split_point in prompt and prot_prompt:
        new_prompt = prompt.replace(
            split_point,
            split_point + ' ' + prot_prompt
        )
    else:
        new_prompt = prompt
    ...
    # tokenize the prompt
    input_ids = tokenizer_multimodal_token(new_prompt, tokenizer, modal_token, return_tensors='pt').unsqueeze(0).long().cuda() #(B, length)
    attention_masks = input_ids.ne(tokenizer.pad_token_id).long().cuda() #(B, length)
...
def mm_infer_batch(image_or_videos, instructs, model, tokenizer, modal='video', miss_prompt="", **kwargs):
    """inference api of VideoLLaMA2 for video understanding.

    Args:
        model: VideoLLaMA2 model.
        image_or_video (torch.Tensor): image tensor (B, 1, C, H, W) / video tensor (B, T, C, H, W).
        instruct (str): text instruction for understanding video.
        tokenizer: tokenizer.
        do_sample (bool): whether to sample.
        modal (str): inference modality.
    ...
    images = []
    for j, instruct in enumerate(instructs):
        # 1. vision preprocess (load & transform image or video).
        if modal == 'text':
            images = None
        else:
            image_or_video = image_or_videos[j]
            if isinstance(image_or_video, dict):
                tensor = {k: v.half().cuda() for k, v in image_or_video.items()}
            else:
                tensor = image_or_video.half().cuda() 
            images.append((tensor, modal))

        # 2. text preprocess (tag process & generate prompt).
        #if isinstance(instruct, str):
        message = [{'role': 'user', 'content': modal_token + '\n' + miss_prompt + instruct}]
        #elif isinstance(instruct, list):
        #    message = copy.deepcopy(instruct)
        #    message[0]['content'] = modal_token + '\n' + message[0]['content']
        #else:
        ...
        input_ids = tokenizer_multimodal_token(prompt, tokenizer, modal_token, return_tensors='pt').unsqueeze(0).long().cuda() #(B, length)
        attention_masks = input_ids.ne(tokenizer.pad_token_id).long().cuda() #(B, length)
