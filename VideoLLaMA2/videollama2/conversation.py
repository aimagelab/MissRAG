...
def get_miss_prompt(modal_type: str, task_modals: List[str]):
    ...
    if len(task_modals)==3 and 'video' in task_modals and 'audio' in task_modals and 'text' in task_modals:
        if modal_type == 'a':
            status += "Video: Missing; Audio: Present; Text: Missing."
        elif modal_type == 'v':
            status += "Video: Present; Audio: Missing; Text: Missing."
        elif modal_type == 't':
            status += "Video: Missing; Audio: Missing; Text: Present."
        elif modal_type == 't':
            status += "Video: Missing; Audio: Missing; Text: Present."
        
        elif modal_type == 'tv':
            status += "Video: Present; Audio: Missing; Text: Present."
        elif modal_type == 'ta':
            status += "Video: Missing; Audio: Present; Text: Present."
        elif modal_type == 'av':
            status += "Video: Present; Audio: Present; Text: Missing."
        elif modal_type == 'tva':
            status += "Video: Present; Audio: Present; Text: Present."
        else:
            raise NotImplementedError
        prompt = status + '\n\n'
        
        #one missing modality
        if modal_type == 'av': #audio+video
            prompt += "The input text is missing. The assistant must use audio and visual data to infer a probable textual context."
        elif modal_type == 'tv': #video+text
            prompt += "The audio is missing. The assistant must use visual and textual data to infer a probable audio context."
        elif modal_type == 'ta': #audio+text
...
def get_prototipe_prompt(modal_type: str, task_modals: List[str], input_text: bool = False):
    status = "Input Modality Status: "
    #audio-video tasks
    if len(task_modals)==2 and 'video' in task_modals and 'audio' in task_modals and not input_text:
        if modal_type == 'v':
            status += "Video: Present; Audio: Prototype."
        elif modal_type == 'a':
            status += "Video: Prototype; Audio: Present."
        else:
            status += "Video: Present; Audio: Present."
    ...
    if len(task_modals)==2 and 'video' in task_modals and 'audio' in task_modals and input_text:
        if modal_type == 'a':
            status += "Video: Prototype; Audio: Present; Text: Prototype."
        elif modal_type == 'v':
            status += "Video: Present; Audio: Prototype; Text: Prototype."
        elif modal_type == 't':
            status += "Video: Prototype; Audio: Prototype; Text: Present."
        
        elif modal_type == 'vt':
            status += "Video: Present; Audio: Prototype; Text: Present."
        elif modal_type == 'at':
            status += "Video: Prototype; Audio: Present; Text: Present."
        elif modal_type == 'av':
            status += "Video: Present; Audio: Present; Text: Prototype."
        ...
        elif modal_type == 'vt': #video+text
            prompt = "The assistant receives an approximate audio data and uses it to generate a response as accurately as possible."
        elif modal_type == 'at': #audio+text
            prompt = "The assistant receives an approximate visual data and uses it to generate a response as accurately as possible."
        #two missing modalities
        elif modal_type == 'v': #video
        ...
        # complete
        elif modal_type == 'avt': #video-audio-text
            prompt = "Audio, visual and textual data are all present."
        else:
            raise NotImplementedError    
    out_prompt = prompt + '\n' + status            
