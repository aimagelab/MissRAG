...
class Videollama2Qwen2ForCausalLM(Qwen2ForCausalLM, Videollama2MetaForCausalLM):
    ...
    def generate(
        ...
        **kwargs,
        ...
        return super().generate(
            ...
            attention_mask=attention_mask,      #(1, len)
            inputs_embeds=inputs_embeds,        #(1, len, 3584)
            **kwargs
        )
    ...
    def extract_modality_tokens(
        ...
        if video is not None:
            video_features = super().encode_images_or_videos([(video, "video")])
        else:
            video_features = None

        if audio is not None:
            audio = audio.unsqueeze(0)
            audio_padding_mask = torch.zeros(audio.shape, device=self.device).bool()  #(B, length, 128)
            audio_features, T, F = super().get_model().get_audio_tower().extract_features(audio, padding_mask=audio_padding_mask, feature_only=True) 
            audio_features = super().get_model().mm_projector_a(audio_features)          #(B, length, 3584)
            audio_features = audio_features.view(1, -1, audio_features.shape[-1])
        else:
            audio_features = None
