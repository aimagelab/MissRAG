...
class Videollama2MetaForCausalLM(ABC):
    ...
    def temporal_aggregator(self, frames_features):
        ...
        # *********** spatial pooling *************
            ...
            video_features = self.get_model().mm_projector(frames_features)
        # *********** time  ************
        elif "tc_connector" in self.config.mm_projector_type or "tp_connector" in self.config.mm_projector_type: #'stc_connector_v35'
            video_features = self.get_model().mm_projector(frames_features)
        else:
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if audio_tower is None:
            ...
            # [tensor, "audio"]
            # [tensor, "video"]
            # [dict("video":tensor, "audio":tensor)]
            
            # Extract video-audio data of each sample in a list
            X_video = []
            X_audio = []
            ...
            if len(X_audio) > 0:
                ...
                audio_padding_mask = torch.zeros(Xa_features.shape, device=self.device).bool()  #(B, length, 128)
                audio_embedding, T, F = self.get_model().get_audio_tower().extract_features(Xa_features, padding_mask=audio_padding_mask, feature_only=True) #(B, length, 768)
                Xa_features = self.get_model().mm_projector_a(audio_embedding)          #(B, length, 3584)
                Xa_features = Xa_features.view(len(X_audio), -1, Xa_features.shape[-1]) #(B, length, 3584)
            # Extract video features
            if len(X_video) > 0:
                X_features = self.encode_images_or_videos(X_video)                      #(B, length, 3584)  

            # Concat video-audio features of each sample
            mm_features = []
            idx_a, idx_v = 0, 0
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            while mm_token_indices.numel() > 0:
                ...
                # Embedd input text before {'<image>': -200, '<video>': -201, '<audio>': -202} and add to the tokens
                cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids[:mm_token_start])) 
                # Add audiovisual features (video+audio features) to the tokens
                ...
                cur_mm_idx += 1
                # Extract input text after {'<image>': -200, '<video>': -201, '<audio>': -202} and verify no other modal tokens are present 
                cur_input_ids = cur_input_ids[mm_token_start+1:] 
                mm_token_indices = torch.where(sum([cur_input_ids == mm_token_idx for mm_token_idx in MODAL_INDEX_MAP.values()]))[0]

            if cur_input_ids.numel() > 0:
                # Embedd input text after {'<image>': -200, '<video>': -201, '<audio>': -202} and add to the tokens
                cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids))
                if labels is not None:
                    cur_new_labels.append(cur_labels)
            cur_new_input_embeds = [x.to(device=self.device) for x in cur_new_input_embeds] #prompt before <video> + audiovisual features + prompt after <video>
            # Final input sequence: text before tag + modality tokens + text after tag 
        ...
        # padding
        if any(x.shape != new_input_embeds[0].shape for x in new_input_embeds): #False
