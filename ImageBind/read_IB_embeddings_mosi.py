...
dataset_prefix = "mosi" # set the name of the correct dataset ("mosi / mosei")
ids_name = "video" 
...
for index, video_token_filename in video_IB_embeddings_files:
    ...
    text_token_filename = f"{dataset_prefix}_text_IB_embeddings_{index}.pt"
    ...
    text_IB_embeddings_np = torch.load(os.path.join(data_dir, text_token_filename))
    ...
    text_IB_embeddings = torch.from_numpy(text_IB_embeddings_np) if isinstance(text_IB_embeddings_np, np.ndarray) else text_IB_embeddings_np
    
    # Collect data
    all_video_IB_embeddings.append(video_IB_embeddings)
    all_audio_IB_embeddings.append(audio_IB_embeddings)
    all_text_IB_embeddings.append(text_IB_embeddings)
...
all_text_IB_embeddings = torch.cat(all_text_IB_embeddings, dim=0)
...
print(f"Total text IB_embeddings shape: {all_text_IB_embeddings.shape}")
print(f"Total number of video IDs: {len(all_video_ids)}")

#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
num_samples = len(all_video_ids)
with h5py.File(f'{root}/IB_embeddings/{split}/IB_embeddings_{split}_{dataset_prefix}.h5', 'w') as h5f:
        ...
        text_ds[:, :] = all_text_IB_embeddings.numpy()
