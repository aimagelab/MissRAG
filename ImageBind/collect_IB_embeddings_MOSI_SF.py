...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--answer_path", type=str, default='/path/to/MissRAG/ImageBind/IB_embeddings/train/mosi_SF'
    )
    ...
    dataset = MOSI(root=args.root, mode=args.mode)
    ...
    for video, audio, text, id in tqdm(dataloader):
        ...
        torch.save(video_tokens_np, f"{args.answer_path}/mosi_video_IB_embeddings_{index}.pt")
        torch.save(audio_tokens_np, f"{args.answer_path}/mosi_audio_IB_embeddings_{index}.pt")
        torch.save(text_tokens_np,  f"{args.answer_path}/mosi_text_IB_embeddings_{index}.pt")
        torch.save(id,              f"{args.answer_path}/mosi_ids_{index}.pt")   
