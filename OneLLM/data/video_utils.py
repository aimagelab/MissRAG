...
class TemporalCropEK(nn.Module):
    """
...
def load_and_transform_video_data(
    ...
    all_video = [video_transform(clip) for clip in all_video] #5 clips (3, 1, H, W)
    all_video = SpatialCrop(224, num_crops=3)(all_video)

    all_video = torch.stack(all_video, dim=0) #15 clips (3, 1, 224, 224)
...
def load_and_transform_video_ek(
    start_frame,
    end_frame,
    video_path,
    num_frames=15,
    ...
    video_transform = transforms.Compose(
        [
            ...
            TemporalCropEK(frames_per_clip=num_frames, stride=40),
            SpatialCropEK(crop_size=224, num_crops=1),
        ]
    )
    ...
    for indice in indices: 
        video_clip = Image.open(os.path.join(video_path, "frame_%010d.jpg" % indice))
        all_video.append(np.array(video_clip))

    all_video = np.stack(all_video)
    all_video = np.transpose(all_video, (0, 3, 1, 2))
    all_video = torch.Tensor(all_video)
    all_video = all_video.transpose(0, 1) #(C=3, num_frames=15, H, W)

    all_video = video_transform(all_video)  #(C=3, num_frames=15, 224, W)
    all_video = torch.stack(all_video, dim=0).squeeze(0)

    all_video = all_video.transpose(0, 1)
