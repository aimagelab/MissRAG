...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--answer_path", type=str, default="/path/to/MissRAG/OneLLM/results/eval_chap_charadesego_retrieval.json", help="Path to save the answer"
    )
    ...
    result = {}
    print("Starting...")
    dataset = CaptionDataset(test_path=args.data_path, video_path=args.video_path, audio_path=args.audio_path, test_IB_path=args.test_IB_embeddings_path)
        ...
        for data in tqdm(dataloader):
            ...
            if 'video' in recover_modal:
                retrieved_video_tokens = retrieve_modality(
                                        IB_index_video,         # index_file
                                        IB_audio.cpu().numpy(), # query
                                        args.k,                 # k
                                        train_video_tokens,     # train_tokens
                                        video.shape[0],         # test_batch_size
                                        30,
                                        4096                                       
                                        )
