...
class MOSEI(Dataset):
    def __init__(self, 
        ...
        self.data = self.data[self.data['mode'] == self.mode] #4659
        self.root = root
...
def audio_visual_generate(images, modal, task_modals, task, text, conv_template_name, user_command, prompt_inside, missing_prompt, use_text_modality=True, oneshot=False):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="""Rate its overall sentiment as a real number between -3.0 (extremely negative) and 3.0 (extremely positive).""",
        help='Template for the prompt'
    )
    parser.add_argument(
    ...
    with torch.no_grad():
        for data in tqdm(dataloader):
            ...
            preds_classification = audio_visual_generate(images=images, modal=args.modal, task_modals=args.task_modals, task='classification', text=text, conv_template_name=args.conversation_template, user_command=args.user_command_classification, prompt_inside=args.prompt_inside, missing_prompt=args.missing_prompt, use_text_modality=args.use_text_modality, oneshot=args.oneshot)
