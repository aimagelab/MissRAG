...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default='/path/to/MissRAG/OneLLM/prototypes/data/modality_tokens_train_MOSEI.h5',
    ...
    parser.add_argument(
        "--answer_path", type=str, default="/path/to/MissRAG/results/eval_sentimentAnalysis_MOSEI_retrieval.json", help="Path to save the answer"
    )
    ...
    if args.debug:
        ...
        for path in other_modality_tokens_path:
            with h5py.File(path, 'r') as h5f:
                ...
                if "text" in h5f:
                    other_text_modality_tokens.append(h5f['text'][:other_limit])
        ...
        with h5py.File(args.train_modality_tokens_path, 'r') as h5f:
            ...
            for path in other_modality_tokens_path:
                with h5py.File(path, 'r') as h5f:
                    ...
                    if "text" in h5f:
                        other_text_modality_tokens.append(h5f['text'][:])
    ...
    train_text_tokens = np.concatenate([train_text_tokens, *other_text_modality_tokens])
    ...
    if args.debug:
        ...
        for path in other_IB_embeddings_path:
            with h5py.File(path, 'r') as h5f:
                ...
                if "text" in h5f:
                    other_text_IB_embeddings.append(h5f['text'][:other_limit])
    else:
        with h5py.File(args.train_IB_embeddings_path, 'r') as h5f:
    ...
    train_IB_text = np.concatenate([train_IB_text, *other_text_IB_embeddings])
    ...
    result = {}
    print("Starting...")
    dataset = MOSEI(root=args.root, test_IB_path=args.test_IB_embeddings_path) 
