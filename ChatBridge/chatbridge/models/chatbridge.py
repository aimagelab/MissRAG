...
class ChatBridge(Blip2Base):
    ...
    def encode_audio(self, audio):
        ...
        with self.maybe_autocast():      
            ...
            inputs_llama = self.llama_proj(query_output.last_hidden_state) #(B, 32, 768)
    ...
    def forward_inference(self, samples):
        task = samples['task']
        if ('tia' in task or 'tva' in task) and 'image' in samples.keys() and 'audio' in samples.keys():
            image = samples["image"]
            img_embeds, atts_img = self.encode_img(image)
        ...
        img_embeds, atts_img = self.process_prompt_instruct_inference(samples['conversation'], feat_list)

        to_regress_tokens = self.llama_tokenizer(['aa'],return_tensors="pt",padding="longest",truncation=True,max_length=self.max_txt_len,add_special_tokens=False)
        batch_size = img_embeds.shape[0]
        bos = torch.ones([batch_size, 1], dtype=to_regress_tokens.input_ids.dtype, device=self.device) * self.llama_tokenizer.bos_token_id
        bos_embeds = self.llama_model.model.embed_tokens(bos) #beginning of sentence

        inputs_embeds = torch.cat([bos_embeds, img_embeds], dim=1) #(B, N, 5120)
        print(f"Input Embeds with bos shape: {inputs_embeds.shape}") 

        atts_bos = atts_img[:, :1] 
        attention_mask = torch.cat([atts_bos, atts_img], dim=1) #(B, N)
        print(f"Att Mask Embeds with bos shape: {attention_mask.shape}")
        ...
        for i, output in enumerate(outputs):
            if output[0] == 0 or output[0] == 1:  # the model might output a unknow token <unk> or a start token <s> at the beginning. remove it
                #outputs[i] = output[1:]
                outputs_clean.append(output[1:])
            else:
                outputs_clean.append(output)

        output_texts = self.llama_tokenizer.batch_decode(outputs_clean, skip_special_tokens=True, clean_up_tokenization_spaces=False, add_special_tokens=False)

        outputs_texts = []
        for response, prompt in zip(output_texts, samples['conversation']):
    ...
    def forward_inference_oneshot(self, samples, allowed_tokens):
        task = samples['task']
        if ('tia' in task or 'tva' in task) and 'image' in samples.keys() and 'audio' in samples.keys():
            image = samples["image"]
            img_embeds, atts_img = self.encode_img(image) #B, 32, 5120
            audio = samples["audio"]
            aud_embeds, atts_aud = self.encode_audio(audio) #B, 32, 5120
            feat_list = [(img_embeds, atts_img), (aud_embeds, atts_aud)]
        elif 'ta' in task and 'audio' in samples.keys():
            audio = samples["audio"]
            img_embeds, atts_img = self.encode_audio(audio)
        ...
        with self.maybe_autocast():
            outputs = self.llama_model(
                inputs_embeds=inputs_embeds, #B, N, 5120
                attention_mask=attention_mask #B, N
            )
        logits = outputs[0]                     #B, N, 32001
        print(f"logits shape: {logits.shape}")
        next_token_logits = logits[:, -1, :] #last token logits
        probs = torch.softmax(next_token_logits, dim=-1)

        allowed_token_probs = probs[0, allowed_tokens]
        normalized_probs = allowed_token_probs / allowed_token_probs.sum()
        token_probs = {token_id: normalized_probs[token_id_idx].item() for token_id_idx, token_id in enumerate(allowed_tokens)}

        most_probable_token_id = max(token_probs, key=token_probs.get)
        most_probable_token_prob = token_probs[most_probable_token_id]
        # Convert token ID to actual token text
        most_probable_token = self.llama_tokenizer.decode(most_probable_token_id)
        return most_probable_token, most_probable_token_prob

    def forward_inference_pepr(self, samples):
        task = samples['task']
        ...
        if ('tia' in task or 'tva' in task) and 'image' in samples.keys() and 'audio' in samples.keys():
            ...
            feat_list = []
        img_embeds, atts_img = self.process_prompt_instruct_inference_pepr(samples['conversation'], feat_list)
    ...
    def forward_inference_retrieval(self, samples):
        task = samples['task']
        if ('tia' in task or 'tva' in task) and \
                ('image' in samples.keys() or 'image' in samples['retrieved_tokens']) and \
                ('audio' in samples.keys() or 'audio' in samples['retrieved_tokens']):
            
            if "image" in samples["retrieved_tokens"]:
                img_embeds = samples["retrieved_tokens"]["image"]
                atts_img = torch.ones(img_embeds.size()[:-1], dtype=torch.long).to(img_embeds.device)
            else:
                image = samples["image"]
                img_embeds, atts_img = self.encode_img(image)
            
            if "audio" in samples["retrieved_tokens"]:
                aud_embeds = samples["retrieved_tokens"]["audio"]
                atts_aud = torch.ones(aud_embeds.size()[:-1], dtype=torch.long).to(aud_embeds.device)
            else:
                audio = samples["audio"]
                aud_embeds, atts_aud = self.encode_audio(audio)
            feat_list = [(img_embeds, atts_img), (aud_embeds, atts_aud)]
        ...
        #     img_embeds, atts_img = self.encode_audio(audio)
        #     feat_list = [(img_embeds, atts_img)]
        elif ('ti' in task or 'tv' in task) and ('image' in samples.keys() or 'image' in samples['retrieved_tokens']):
            if "image" in samples["retrieved_tokens"]:
                img_embeds = samples["retrieved_tokens"]["image"]
                atts_img = torch.ones(img_embeds.size()[:-1], dtype=torch.long).to(img_embeds.device)
            else:
    ...
    def forward_inference_retrieval_oneshot(self, samples, allowed_tokens):
        task = samples['task']
        if ('tia' in task or 'tva' in task) and \
                ('image' in samples.keys() or 'image' in samples['retrieved_tokens']) and \
                ('audio' in samples.keys() or 'audio' in samples['retrieved_tokens']):
    ...
    def process_prompt_instruct(self, conversations, feat_list):
        ...
        for idx, conversation in enumerate(conversations): #0,1,...,15
            ...
            for i in range(len(feat_list)): # 0,1 = [(img_embeds, atts_img), (aud_embeds, atts_aud)]
    ...
    def process_prompt_instruct_inference(self, conversations, feat_list):
        inputs_embeds = []
        attention_masks = []
        ...
        for idx, conversation in enumerate(conversations): #0,1,...,15
            ...
            if '<query>' in conversation:
                ...
                prompts = [conversation]  # Handle the case where <query> is not present: only-text task
            ...
            for i in range(len(feat_list)): # 0,1 = [(img_embeds, atts_img), (aud_embeds, atts_aud)]
                ...
                inputs_embeds_list.append(prompti_embeds) #prompt_i = 'Given following video: ' /  'and its background audio: '
                inputs_embeds_list.append(inputs_llama) #modality_i = <video_embd> / <audio_embd>
                attention_mask_list.append(prompti_atts) #att_prompt_i
                attention_mask_list.append(atts_llama.to(inputs_llama.device)) #att_modality_i

            pt1 = prompt = prompts[-1] # '.###Human: <human instruction>###Assistant:'
            #start_token = '###Assistant:'
            #while start_token in prompt:
                #start_idx = prompt.find(start_token)+len(start_token)
                #pt1 = prompt[:start_idx] #entire prompt
                #prompt = prompt[start_idx:] #void
                
            pt1_embeds, pt1_atts, pt_ids = self.encode_pt(pt1, device) 
            inputs_embeds_list.append(pt1_embeds) #last_prompt
            attention_mask_list.append(pt1_atts) #last_prompt_attn
        ...
        print(f"Inputs_Embed shape after padding:{inputs_embed.shape}")
        print(f"attention_mask shape after padding:{attention_mask.shape}")
        
        return inputs_embeds, attention_masks

    def process_prompt_instruct_inference_pepr(self, conversations, feat_list):
        inputs_embeds = []
        attention_masks = []

        for idx, conversation in enumerate(conversations): #0,1,...,5
            inputs_embeds_list = []
            attention_mask_list = []
            prompt = conversation
            ...
            prompt_embeds, prompt_atts, prompti_ids = self.encode_pt(prompt, device)
            inputs_embeds_list.append(prompt_embeds) 
            attention_mask_list.append(prompt_atts) 
            #pt1 = prompt = prompt[-1] # '.###Human: <human instruction>###Assistant:'
            #start_token = '###Assistant:'
            #while start_token in prompt:
                #start_idx = prompt.find(start_token)+len(start_token)
                #pt1 = prompt[:start_idx] #entire prompt
