...
def parse_args():
    ...
    parser.add_argument("--modal", nargs='+', type=str, default=[])
    parser.add_argument("--task_modals", nargs='+', type=str, default=['video', 'audio'])
    parser.add_argument("--use_text_modality", action='store_true', default=False, help='Enable or disable the text input modality. Accepts true/false.')
    parser.add_argument("--answer_path", type=str, default="/path/to/MissRAG/results_chatbridge/eval_mosei_retrieval.json")
    parser.add_argument(
        "--train_modality_tokens_path",        
        type=str,
        default='/path/to/MissRAG/ChatBridge/prototypes/data/modality_tokens_train_MOSEI_corrected.h5',
        help= 'Path to the train modality tokens'
    )
    parser.add_argument(
        "--test_IB_embeddings_path", 
        type=str, 
        default='/path/to/MissRAG/ImageBind/IB_embeddings/test/IB_embeddings_test_mosei.h5'
    )
    parser.add_argument(
        "--train_IB_embeddings_path",        
        type=str,
        default='/path/to/MissRAG/ImageBind/IB_embeddings/train/IB_embeddings_train_mosei.h5',
...
class MOSEI(Dataset):
    def __init__(self, vis_processor, aud_processor, test_IB_path, text_processor=None, mode: str = "test", root: str = "/path/to/MOSEI") -> None:
        super().__init__()
        self.mode = mode
...
if __name__ == "__main__":
    ...
    dataset = MOSEI(root=args.root, test_IB_path=args.test_IB_embeddings_path, vis_processor=vis_processor, aud_processor=aud_processor)   
    ...
    with torch.no_grad():
        for data in tqdm(dataloader):
            ...
            if args.one_shot:
                result, prob = model.forward_inference_retrieval_oneshot(samples=samples, allowed_tokens=[6374, 21104, 8178])
                print(f"Answer: {result}, Probability: {prob}")
                predictions.append({
                    'image_id': id[0],
