...
def parse_args():
    ...
    parser.add_argument(
        "--answer_path", type=str, default="/path/to/MissRAG/ChatBridge/prototypes/data/modality_tokens/train/MOSI_SF"
    )
    parser.add_argument(
...
class MOSI(Dataset):
    def __init__(self, vis_processor, aud_processor, text_processor=None, mode: str = "train", root: str = "/path/to/MOSI") -> None:
        super().__init__()
        self.mode = mode
...
if __name__ == "__main__":
    ...
    dataset = MOSI(vis_processor=vis_processor, aud_processor=aud_processor, root=args.root)   
    ...
    with torch.no_grad():
        for sample in tqdm(dataloader):
            ...
            torch.save(image_tok_np, f"{args.answer_path}/MOSI_video_tokens_{index}.pt")
            torch.save(audio_tok_np, f"{args.answer_path}/MOSI_audio_tokens_{index}.pt")
            torch.save(text, f"{args.answer_path}/MOSI_text_{index}.pt")
            torch.save(id, f"{args.answer_path}/MOSI_SF/MOSI_ids_{index}.pt")   
