...
def parse_args():
    ...
    parser.add_argument(
        "--answer_path", type=str, default="/path/to/MissRAG/ChatBridge/prototypes/data/modality_tokens/train/valor_SF"
    )
    parser.add_argument(
...
class CaptionDataset(Dataset):
    def __init__(self, vis_processor, aud_processor, data_path, root) -> None:
        super().__init__()
...
if __name__ == "__main__":
    ...
    dataset = CaptionDataset(vis_processor=vis_processor, aud_processor=aud_processor, data_path=args.data_path, root=args.root)   
    ...
    with torch.no_grad():
        for video, audio, video_ids in tqdm(dataloader):
            ...
            torch.save(image_tok_np, f"{args.answer_path}/valor_video_tokens_{index}.pt")
            torch.save(audio_tok_np, f"{args.answer_path}/valor_audio_tokens_{index}.pt")
