...
def parse_args():
    ...
    parser.add_argument(
        "--one_shot", action='store_true', default=False
    )
    parser.add_argument(
...
class MOSEI(Dataset):
    def __init__(self, vis_processor, aud_processor, text_processor=None, mode: str = "test", root: str = "/path/to/MOSEI") -> None:
        super().__init__()
        self.mode = mode
...
if __name__ == "__main__":
    ...
    dataset = MOSEI(root=args.root, vis_processor=vis_processor, aud_processor=aud_processor)   
    ...
    with torch.no_grad():
        for data in tqdm(dataloader):
            ...
            if args.one_shot:
                ...
                result, prob = model.forward_inference_oneshot(samples=samples, allowed_tokens=[6374, 21104, 8178])
                ...
                for video_id, result_classification in zip(id, results): 
                    predictions.append({
                        'image_id': video_id,
